...
void binaryInsert(Ref &pRoot, Ref p){
    ...
    while(trav != nullptr){
        prev = trav;
        if(p->key < trav->key) trav = trav->left;
        else if(p->key > trav->key) trav = trav->right;
...
void leftRotate(Ref &pRoot, Ref p, Ref nil){
    ...
    q->parent = p->parent;
    if(p->parent == nil) pRoot = q;// pRoot = p so after rotating, pRoot change to q
    else{
...
void rightRotate(Ref &pRoot, Ref p, Ref nil){// similar
    Ref q = p->left;
    p->left = q->right;
    if(q->right != nil){
...
void RBTree_FixUp(Ref &pRoot, Ref x, Ref nil){
    ...
    if(p->color == RED && u->color == RED){// Case 2: Swap g color with p, u color
        g->color = RED;
